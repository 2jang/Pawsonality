...
export default function ChatbotPage() {
  ...
  const pawnaType = location.state?.pawnaType;

  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content: pawnaType
        ? `안녕하세요! 🐾 ${pawnaType} 유형에 대해 궁금하신 점이나 양육 방법에 대해 무엇이든 물어보세요!`
        : "안녕하세요! 🐾 Pawsonality 챗봇입니다. 강아지 양육에 대해 무엇이든 물어보세요!",
      timestamp: new Date(),
    },
  ]);
  const [input, setInput] = useState("");
  const [pawna, setPawna] = useState(pawnaType || "");
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            <div className="flex items-center gap-4">
              <label className="text-sm font-medium">Pawna 유형 (선택)</label>
              <input
                type="text"
                value={pawna}
                onChange={(e) => setPawna(e.target.value.toUpperCase())}
                placeholder="예: CNEA"
                maxLength={4}
